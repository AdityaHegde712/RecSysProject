...
LightGCN-HG retained as a secondary because it addresses the "use the features"
critique through a channel orthogonal to SASRec (geography, not time) and
adds a graph-based research angle to the team comparison.
...
| Max sequence length | 50, 100, 200 | **100** | Covers the 95th-percentile user history on the 20-core (≈ 60 items); 200 just adds padding. |
...
Total training time **869 s** (~14.5 min on one RTX 5070 Ti).
...
Metadata extracted by `scripts/extract_hotel_meta.py` (read-only over
`item2id.json`, never touches the shared 20-core splits):
