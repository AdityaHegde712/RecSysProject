"""
Train GMF baseline on HotelRec.
...
def train(config, kcore_dir, num_users, num_items):
    ...
    # Try CUDA, fall back to CPU if unavailable or incompatible
    ...
    model = GMF(num_users, num_items, embed_dim=embed_dim).to(device)
...
def main():
    ...
    parser.add_argument("--config", default="configs/gmf.yaml")
